# TryHackMe Write-ups

Write-ups focused on practical cybersecurity learning, analysis, and methodology (not step-by-step solutions).

## Index

- [OhSINT](./ohsint.md) — Beginner OSINT room focused on metadata, username pivoting, and public-source correlation.
- [SoMeSINT](./somesint.md) — A social media OSINT room focused on username pivoting, archived pages, source inspection, and   cross-platform profiling.
- [Input Manipulation & Prompt Injection](./input-manipulation-prompt-injection.md) — A beginner-friendly introduction to prompt injection, system prompt leakage, jailbreaking, and LLM security risks.
- [Juicy Details](./juicy-details.md) — Beginner SOC-style investigation using web, auth, and FTP logs to trace attacker activity.
- [OWASP Top 10 2025 – Application Design Flaws](./owasp-top10-2025-application-design-flaws.md)
